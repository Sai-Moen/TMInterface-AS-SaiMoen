...
PluginInfo@ GetPluginInfo() {
    ...
    info.Author = "Tomashu";
    info.Version = "v1.0.2";
    info.Description = "Shows speedslide quality as a slider";
    return info;
}
*/
...
float GetSpeedslideQualityForStadiumCar(float angvel_y, float speed_x, float speed_z, float material_max_side_friction_multiplier = 1.0) {
    ...
    float sidefriction = 20 * speed_x;

    float delta = Math::Abs(angvel_y) * 29.8759102821;

    if (Math::Abs(sidefriction - delta) < maxsidefriction) {
        sidefriction += delta;
    }
...
float ComputeSpeedslideQualityForStadiumCar(SimulationManager@ simManager) {
    ...
    const auto@ const currentState = simManager.Dyna.RefStateCurrent;
    mat3 rotation = currentState.Location.Rotation;
    vec3 local_speed = MultTranspose(currentState.LinearSpeed, rotation);
    vec3 local_angvel = MultTranspose(currentState.AngularSpeed, rotation);
    return GetSpeedslideQualityForStadiumCar(
       local_angvel.y, local_speed.x, local_speed.z,
       material_max_side_friction_multiplier);
}
